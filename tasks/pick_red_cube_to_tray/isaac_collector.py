"""
tasks/pick_red_cube_to_tray/isaac_collector.py
Entry point: pick up the red cube and place it on the tray.

Usage (Windows desktop):
  <isaac_sim_root>\\python.bat tasks\\pick_red_cube_to_tray\\isaac_collector.py ^
      --output data\\pick_red_cube_to_tray\\demos.hdf5 --num_episodes 300 --scripted
"""
...
class PickRedCubeToTrayScene(PickAndPlaceScene):
...
if __name__ == "__main__":
    args = parse_collector_args(
        default_output      = "data/pick_red_cube_to_tray/demos.hdf5",
        default_instruction = "pick up the red cube and place it on the tray",
    )
    scene = PickRedCubeToTrayScene(camera_ids=args.camera)
    collect(scene, args)
