...
class Stage1Trainer(BaseTrainer):
    """
    LeWorldModel-style Stage 1: train TokenEncoder + Predictor + TokenDecoder.

    The predictor now runs in compact D_compact space instead of raw D_vit space.
    Three losses (Phase 1):
      L_pred  — teacher-forcing: predict next compact latent delta
      L_kl    — KL( N(μ,σ²) || N(0,I) ) — prevents representation collapse
      L_recon — TokenDecoder(μ_t) ≈ z_t  — keeps compact space informative

    Phase 2 (enable by setting lambda_roll > 0 and resuming):
      L_roll  — 2-step autoregressive rollout in compact space
    """
    ...
    def __init__(
        ...
        # Wrap all three Stage 1 modules with DDP
        ...
        all_params = (
            ...
            list(self._unwrap(self.system.predictor).parameters())     +
            list(self._unwrap(self.system.token_decoder).parameters())
        )
        self.optimizer = optim.AdamW(
            all_params, lr=config["learning_rate"], weight_decay=0.05,
        )
    ...
    def _encode_clip(self, frames: torch.Tensor) -> torch.Tensor:
        """frames : (B, T, C, H, W) → tokens : (B, T, N, D_vit)"""
    ...
    def _run_epoch(self, loader, train: bool, epoch: int) -> dict:
        self._unwrap(self.system.token_encoder).train(train)
        self._unwrap(self.system.predictor).train(train)
        self._unwrap(self.system.token_decoder).train(train)
        if train and hasattr(loader.sampler, "set_epoch"):
            loader.sampler.set_epoch(epoch)

        totals = {"pred": 0.0, "sig": 0.0, "recon": 0.0, "roll": 0.0,
                  "total": 0.0, "cos_pred": 0.0, "cos_repeat": 0.0}
        n     = 0
        phase = "train" if train else "val"
        pbar  = tqdm(loader, desc=f"  {phase} ep {epoch}",
                     disable=not self.is_main, leave=False, dynamic_ncols=True)

        enc  = self._unwrap(self.system.token_encoder)
        pred = self._unwrap(self.system.predictor)
        dec  = self._unwrap(self.system.token_decoder)

        lam_pred  = self.config.get("lambda_pred",  1.0)
        lam_sig   = self.config.get("lambda_sig",   0.01)
        sigreg_M  = self.config.get("sigreg_M",     64)
        lam_recon = self.config.get("lambda_recon", 1.0)
        lam_roll  = self.config.get("lambda_roll",  0.0)

        with torch.set_grad_enabled(train):
            for frames, actions, _poses in pbar:
                frames  = frames.to(self.device)   # (B, T, C, H, W)
                actions = actions.to(self.device)  # (B, T, action_dim)

                # ── Frozen ViT encoding ───────────────────────────────── #
                z = self._encode_clip(frames).detach()  # (B, T, N, D_vit)
                B, T, N, D_vit = z.shape

                # ── Compact encoding for all T frames ────────────────── #
                z_flat = z.reshape(B * T, N, D_vit)
                c_flat = enc(z_flat)                         # (B*T, N, D_compact)
                c      = c_flat.reshape(B, T, N, -1)        # (B, T, N, D_compact)

                # ── L_sig : SIGReg on mean-pooled compact tokens ──────── #
                # Pool over patches → (B*T, D_compact), n=B*T samples
                Z_pool = c_flat.mean(dim=1)                  # (B*T, D_compact)
                L_sig  = self.system.__class__.sigreg(Z_pool, M=sigreg_M)

                # ── L_recon : decode back to ViT token space ──────────── #
                z_recon = dec(c_flat)                        # (B*T, N, D_vit)
                L_recon = F.mse_loss(z_recon, z_flat.detach())

                # ── L_pred : teacher-forcing in compact space ──────────── #
                # Target: delta between consecutive compact tokens (stop-grad)
                c_pred_tf = pred.forward_clip(
                    c, actions[:, :-1]
                )                                            # (B, T-1, N, D_compact)
                delta_tgt = (c[:, 1:] - c[:, :-1]).detach()
                L_pred    = F.mse_loss(c_pred_tf, delta_tgt)

                # ── L_roll : 2-step rollout (Phase 2, lambda_roll > 0) ── #
                L_roll = torch.tensor(0.0, device=self.device)
                if lam_roll > 0.0 and T >= 3:
                    z_hat_2, delta_3 = pred.rollout_2step(
                        c, actions[:, :2], gamma=1.0
                    )
                    L_roll = F.mse_loss(
                        z_hat_2 + delta_3,
                        c[:, 2].detach(),
                ...
                # ── Metrics ───────────────────────────────────────────── #
                with torch.no_grad():
                    c_next_pred = (c[:, :-1] + c_pred_tf.detach())
                    cos_pred    = F.cosine_similarity(
                        c_next_pred.float(), c[:, 1:].float(), dim=-1
                    ).mean().item()
                    cos_repeat  = F.cosine_similarity(
                        c[:, :-1].float(), c[:, 1:].float(), dim=-1
                    ).mean().item()

                totals["pred"]       += L_pred.item()
                totals["sig"]        += L_sig.item()
                totals["recon"]      += L_recon.item()
                totals["roll"]       += L_roll.item()
                totals["total"]      += loss.item()
                totals["cos_pred"]   += cos_pred
                totals["cos_repeat"] += cos_repeat
                n += 1
                pbar.set_postfix(
    ...
    def train(self):
        ...
        if self.is_main:
            print(f"\n[Stage1] LeWorldModel predictor training  "
        ...
        for ep in range(self.start_epoch, epochs + 1):
            ...
            if self.is_main:
                ...
                roll_str = f"  val_roll={vl['roll']:.4f}" \
                           if self.config.get("lambda_roll", 0.0) > 0 else ""
                print(
                    f"[Stage1] ep {ep:3d}/{epochs}  "
                    f"val_pred={vl['pred']:.4f}  "
                    f"val_sig={vl['sig']:.4f}  "
                    f"val_recon={vl['recon']:.4f}"
                    f"{roll_str}  "
                    f"cos_pred={vl['cos_pred']:.3f}  "
                    f"cos_repeat={vl['cos_repeat']:.3f}  "
                    f"lr={lr:.2e}"
                )
                if vl["total"] < best:
                    best = vl["total"]
